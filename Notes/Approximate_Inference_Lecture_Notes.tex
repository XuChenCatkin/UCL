...
\title{COMP0085: Approximate Inference - Lecture Notes}
...
\subsection{Variables are Neighbours}

\begin{remark}[Neighbourhood Relationship]
Variables are neighbours if they share a common factor, meaning there exists a factor that references both variables.
\end{remark}
...
Can you get a ball from $X$ to $Y$ without being blocked by $V$? This determines conditional independence relationships:

\begin{itemize}
    \item \textbf{Nodes } $V \in V$ pass balls down or up chains: $\Rightarrow V \to$ or $\leftarrow V$
    
    \item \textbf{Nodes } $V \notin V$, bounce balls from children to children via parent $V$ (nodes receive via children-to-children paths where paths continue)
\end{itemize}

\textbf{Blocking rule in DAG:} $P_a(X) \cup d(X) \cup \text{pa}(X)$?
\end{remark}

\subsection{Markov Boundary for X in DAG}

\begin{remark}[Markov Boundary in Directed Acyclic Graphs]
The Markov boundary $\{ \text{pa}(X) \cup \text{ch}(X) \cup \text{pa}(\text{ch}(X)) \}$?

Every graph $G$ implies conditional independence statements (CIS) $-\{X_i \perp Y | U\}$ for i.i.d.

For each set $C$, we can define a family of distributions that satisfy all constraints in $C$. We have:
\[
p_C(\theta) = \{ P(Z) : P(X_i, Y_f | V_f) = P(Z | V_f) P(Y_f | U) \text{ for } \forall \text{ } Y_f, V | U \text{ in } C \}
\]

Independence view: $G$ may also encode a family of distributions by functional form:
\[
P_G = \{ P(Z) : P(Z) = \frac{1}{Z} \prod_i f_i(V_{C_i}) \text{ for some non-negative functions } f_i \}
\]

\begin{itemize}
    \item For directed graphs: $P_G = p_C(\theta)$ — $P(Z) = \prod_i P(X_i | \text{pa}(i))$ — global Markov $\Leftrightarrow$ factorization
    
    \item For undirected graphs: $P_G = p_C(\theta)$ if all distributions are positive, i.e., $p(z) > 0$ for all $z$ (Hammersley-Clifford Theorem) — $P(Z) = \frac{1}{Z} \prod_i f_i(Z_{C_i})$ by cliques
    
    \item For factor graphs, there are some cases $P_G \neq p_C(\theta)$
    
    \item \textbf{Expressive advantage:} Every undirected model can be written as a factor graph, but not every factor graph collapses to an undirected graph with same family.
\end{itemize}
\end{remark}

\subsection{Adding Edges and Conditional Independence}

\begin{remark}[Edge Addition Effects]
Adding edges to a graph $\Rightarrow$ removing conditional independence statements $\Rightarrow$ conditional independence set shrinks $\Rightarrow$ family of distributions enlarges. Denser graphs being more expressive but less constrained in conditional independence.
\end{remark}
...
\section{Advanced Models: State Machines and Boltzmann Machines}
...
\begin{definition}[Extended Kalman Filter (EKF)]
...
where $W_t, V_t$ are typically still Gaussian. Rewriting with linearized coefficients:
\begin{align*}
Z_{t+1} &\approx \tilde{D}_z u_t + \tilde{A}_z (Z_t - \hat{Z}_t^*) + W_t \\
X_t &\approx \tilde{D}_z u_t + \tilde{C}_z (Z_t - \hat{Z}_t^*) + V_t
\end{align*}

The exogenous input $u_t$ is treated as a fixed realized value (observed). For prediction, we use:
\[
\hat{Z}_{t+1}^* = f(\hat{Z}_t^*, u_t), \quad \hat{X}_t^* = g(\hat{Z}_t^*, u_t)
\]

This allows approximate inference in nonlinear systems via the EKF and particle filters.
\end{definition}
