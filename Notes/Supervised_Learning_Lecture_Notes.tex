...
\title{COMP0078: Supervised Learning - Lecture Notes}
...
These lecture notes cover the fundamentals and advanced techniques in supervised learning. The course develops both theoretical understanding and practical methods for learning predictive models from labeled data.
...
\subsection{Weak Inverse (Pseudoinverse)}

\begin{definition}[Pseudoinverse]
The pseudoinverse of $\Phi \in \mathbb{R}^{n \times D}$ is a matrix $\Phi^\dagger \in \mathbb{R}^{D \times n}$ such that:
\[
...
\subsection{Instability of Pseudoinverse on Small Principal Directions}

\begin{remark}[Pseudoinverse Instability]
Let $\Phi = U \Sigma V^T$ with rank $r \leq n$ and smallest singular value $\sigma_r = 10^{-8}$. Then:
\[
\hat{w} = V\Sigma^+ U^T y = V\Sigma^{-1} U^T y
\]

Let $x = U_n$ be the $n$-th column of $V$. Let $x' = (I+H)x$ and see:
\[
\|\hat{f}(x()) - \ell(x')\| = |\hat{w}^T x - w^T c(+z)x| = |z| \left|\frac{U_n^T \Sigma^+ U^T y}{\sigma_p}\right| = 10^8 z \left|\langle w, u_z\right|\right|
\]

(Indicates small weights)
\end{remark}
...
\begin{definition}[Ridge Regression with Regularization]
Ridge regression adds an $\ell_2$ penalty to prevent overfitting:
\[
\hat{w}_\lambda = \arg\min_w \frac{1}{n} \sum_{i=1}^n (\ell(f(x_i)) - y_i)^2 + \lambda \|w\|^2 \quad \text{(make } w \text{ not too large)}
\]

The solution is:
\[
\hat{w}_\lambda = V\Sigma^+ U^T y = (\Sigma^\Sigma + \lambda I)^{-1} \Sigma^T y
\]

The smallest singular value is always larger than:
\begin{itemize}
    \item Since eigenvalues of $\Sigma^T\Sigma$ are $\{\mu_i + \lambda\}$ 
    \item And $\Sigma^T\Sigma + \lambda I$ has eigenvalues $\sigma_i^2 + \lambda$
    \item So $(\Sigma^\Sigma + \lambda I)^{-1}$ has $\frac{\sigma_i^2}{\sigma_i^2 + \lambda} \ll \frac{1}{\sigma_i}$
\end{itemize}
\end{definition}
...
\subsection{M-Smoothness Condition}

\begin{definition}[M-Smoothness]
A function $F: \mathbb{R}^d \to \mathbb{R}$ is $M$-smooth if it has Lipschitz continuous gradient, with Lipschitz constant $M$:
\[
\|\nabla F(w) - \nabla F(z)\| \leq M \|z - w\| \quad \forall w, z \in \mathbb{R}^d
\]
\end{definition}
...
Let $F: \mathbb{R}^d \to \mathbb{R}$ be a differentiable and $M$-Lipschitz function. By the definition of subgradients:
\[
\|F(\mathbf w) - \nabla F(\mathbf z)\|^* \leq 2 \|w - z\|
\]

\textbf{Note:} The set of all subgradients of $F$ in $w$ is the sub-differential and is denoted as $\partial F(w)$.
\end{definition}

\subsection{Subgradient Concept}

\begin{definition}[Subgradient Method]
For L-Lipschitz function $F$, let $(w_k)_{k=1}^{\infty}$ be a sequence obtained via subgradient method:
\[
F(w_k^{\text{opt}}) - F(w_*) \leq \frac{\|w_0 - w_*\|^2}{2\sum_{k} D_k} + \frac{L^2}{2} \sum_{k} D_k
\]

If we choose $\eta = \frac{1}{\sqrt{E}}$, then we have:
\[
F(w_k^{\text{opt}}) - F(w_*) \leq \frac{1}{2\sqrt{E}} \left(\|w_0 - w_*\| + L^2\right)
\]
\end{definition}
...
In classification trees, we can choose different $\varOp(T)$ of node impurity:

\begin{enumerate}
    \item \textbf{Misclassification error}: $(1 - P_{p,p(p)})$ where $\mathbb{1}[x_i \in \mathcal{R}_p \neq P_{pkp}$ (In each region, only pick the highest one and rest is misclassified)
    
    \item \textbf{Gini index}: $\sum_k P_{pk}(1 - p_k)$
    
    \item \textbf{Cross entropy}: $\sum_k P_{pk} \log \frac{1}{P_{pk}} = -\sum_k p_k \log p_k$
\end{enumerate}

Cross entropy and Gini index are used to grow the tree. They are more sensitive to changes in the node probabilities. They have higher derivative around $0 >> 0.1$ and $0.9 >> 1.0$, encourage selection, but smaller around $0.5$.

Classification error is often used to prune the tree.
\end{definition}
...
\begin{tabular}{|c|c|}
\hline
$t$ & 1, 2, 3, \ldots, T \quad \text{(time)} \\
\hline
$i$ & $\frac{1}{m}$ \\
\hline
2 & $\frac{1}{m}$ & $D_t(i) = \frac{D_t^t e^{-\alpha_t; h_t(x_i)}}$ & $y_i h_t(x_i) \leq 0$ of misclassified, and $e^{-\alpha_t; h_t(x_i)} > 0$, increase weight \\
\hline
\vdots & \vdots & \\
\hline
$m$ & $\frac{1}{m}$ & & \\
\hline
(data) & \multicolumn{2}{|c|}{$\alpha_t$: weight on weak learner $t$, $\alpha_t = \frac{1}{2} \log \frac{1-\varepsilon_t}{\varepsilon_t}$ ($2\varepsilon_t, h_t : ~ f(x) := \sum \alpha_t h_t(x)$)} \\
\hline
& & $h_t$: weak learner generated at time $t$ & $H(x) = \text{sign}(f(x))$ \\
\hline
& & $\varepsilon_t$: weighted error of $h_t$ at $t$, $\varepsilon_t = \sum_{i=1}^m D_t(i) \mathbb{1}\left[\frac{h_t(x_i) \neq y_i}{}\right]$ \\
\hline
\end{tabular}
\end{center}
\end{definition}
