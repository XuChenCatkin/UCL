...
\section{Markov Chain Properties and Limit Theorems}

\subsection{Accessibility and Communication}

\begin{definition}[Markov Chain Transitions]
A Markov chain is characterized by transition probabilities $P_i$, where each element $P_{ij}$ represents the probability of transitioning from state $i$ to state $j$.
\end{definition}

\begin{definition}[Accessibility and Communication]
State $j$ is said to be \textbf{accessible} from state $i$ (denoted $i \to j$) if $P_{ij}^n > 0$ for some $n \geq 0$. 

States $i$ and $j$ are said to \textbf{communicate} (denoted $i \leftrightarrow j$) if both $i \to j$ and $j \to i$ hold.

A Markov chain is said to be \textbf{irreducible} if all states communicate with each other (there is only one communicating class).
\end{definition}

\subsection{Recurrence and Transience}

\begin{definition}[Recurrence]
Let $f_i$ denote the probability that starting in state $i$, the process will ever return to state $i$. Then:
\[
f_i^{(n)} = P(\text{return to } i \text{ in exactly } n \text{ steps}), \quad f_i = \sum_{n=1}^{\infty} f_i^{(n)}
\]

State $i$ is said to be \textbf{recurrent} if $f_i = 1$ (the process will definitely return). Otherwise it is \textbf{transient} ($f_i < 1$).

If states $i$ and $j$ communicate ($i \leftrightarrow j$), then $i$ is recurrent if and only if $j$ is recurrent.
\end{definition}

\subsection{Periodicity}

\begin{definition}[Periodicity of a Markov Chain]
The \textbf{period} of state $i$ is:
\[
d(i) = \text{g.c.d}\{n \geq 1 : P_{ii}^n > 0\}
\]
...
\begin{remark}[Periodicity is a Class Property]
If states $i$ and $j$ communicate ($i \leftrightarrow j$), then $d(i) = d(j)$.
\end{remark}

\subsection{Ergodicity and Stationary Distribution}

\begin{definition}[Ergodicity]
A Markov chain is \textbf{ergodic} if it is irreducible and aperiodic. For a finite-state Markov chain, being irreducible and aperiodic guarantees ergodicity.
\end{definition}

\begin{theorem}[Convergence to Stationary Distribution]
For an ergodic, irreducible Markov chain, there exists a unique stationary distribution $\boldsymbol{\pi}$ such that:
\[
...
\[
\boldsymbol{\pi}^T = \boldsymbol{\pi}^T P, \quad \sum_i \pi_i = 1
\]

Convergence is exponentially fast, with rate determined by the spectral gap (difference between the largest and second-largest eigenvalues).
\end{theorem}

\begin{remark}[Mean Recurrence Time and Ergodic Theorem]
The mean recurrence time of state $i$ is:
\[
M_i = \mathbb{E}[R_i \mid S_0 = i] = \sum_{k=1}^{\infty} k P_i^{(k)} = \frac{1}{\pi_i}
\]

For an ergodic chain, $M_i < \infty$. The ergodic theorem states that the time-average converges to the ensemble average (the stationary distribution):
\[
\lim_{N \to \infty} \frac{1}{N} \sum_{t=1}^N f(S_t) = \mathbb{E}_{\pi}[f(S)] \text{ almost surely}
\]
\end{remark}

\subsection{Importance Sampling with Unknown Normalizers}

\begin{theorem}[Self-Normalized Importance Sampling]
When we have unnormalized densities $\tilde{p}(x) = Z_p p(x)$ and $\tilde{q}(x) = Z_q q(x)$, we can compute the ratio of normalizers:
\[
\frac{Z_p}{Z_q} = \mathbb{E}_q\left[\frac{\tilde{p}(x)}{\tilde{q}(x)}\right]
\]

Thus, for expectations under $p$:
\[
\mathbb{E}_p[f(x)] \approx \frac{\sum_{i=1}^n f(x_i) \tilde{p}(x_i) / \tilde{q}(x_i)}{\sum_{i=1}^n \tilde{p}(x_i) / \tilde{q}(x_i)}
\]

The weights are normalized to sum to 1, making the method practical when normalizing constants are unknown.
\end{theorem}

\remark{Trade-off}
Importance sampling is flexible but can be inefficient when the proposal distribution is far from the target, particularly in high dimensions. When weights are very imbalanced (some samples have very high weight while others have negligible weight), many samples are effectively wasted.
...
A random field has the \textbf{Markov property} if:
\[
p(c_i|s_{\backslash i}, j \in \partial(i)) = p(c_i|s_{\partial i}, j \in \partial(i))
\]
Each variable $\theta_i$ is conditionally independent of the remaining field given its Markov blanket.
\end{definition}
...
To implement this:
\begin{enumerate}
    \item Generate $X^* \sim U[-1,1]$ and $U_i \sim \text{Unif}(0,1)$
    \item If $U_i \leq A(x'|x)$, accept and set $X_{i+1} = x^*$
    \item Otherwise, keep $X_{i+1} = X_i$
\end{enumerate}
\end{definition}
...
\begin{remark}[Theoretical Result on Gibbs]
The conditional densities satisfy:
\[
\pi(x \to x') = p(x_d | x_{\backslash d}), \quad p(x \to x') = p(x_d | x_d) = p(x_d | x_d) \text{ (rest didn't change when we sample } X_d \to X_d^*)
\]
\[
\pi(x) = p(x_d, x_{\backslash d}) = p(x_d | x_{\backslash d}) p(x_{\backslash d}), \quad \pi(x') = p(x_d | x_{\backslash d}) p(x_{\backslash d})
\]
\[
\text{so ratio} \frac{\pi(x') \pi(x \to x')}{\pi(x) \pi(x \to x')} = \frac{p(x_d | x_{\backslash d}) p(x_{\backslash d}) p(x_d | x_d)}{p(x_d | x_{\backslash d}) p(x_{\backslash d}) p(x_d | x_d)} = 1
\]
\end{remark}
